...
with col_settings:
    ...
    # Store settings in session state for backend use (aspect_ratio, formatting, color_guidance, exclude_items will be set in right panel)
...
with col_chat:
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        img_url = data.get(
                            "image_url",
                            "https://placehold.co/400x300?text=Gemma+4+Image"
                        )
    ...
    for prompt, img_url in reversed(st.session_state['chat_history'][-5:]):
        ...
        with response_col2:
            try:
                # Prepend backend URL if img_url is a relative path
                if img_url.startswith("/generated_images/"):
                    full_url = backend_url + img_url
                else:
                    full_url = img_url
        ...
        for idx, (prompt, img_url) in enumerate(reversed(st.session_state['chat_history'])):
            ...
            with col:
                try:
                    ...
                    response = requests.get(full_url)
                    img = Image.open(BytesIO(response.content))
                    st.image(img, caption=prompt, width=None)
                except Exception:
                    st.warning("Could not load image.")
